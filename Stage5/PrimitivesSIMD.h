...
__forceinline __m256i operator == (const __m256i x, const __m256i y) { return _mm256_cmpeq_epi32(x, y); }
...
//__forceinline __m256 my_pow(__m256 x, __m256 y)
//{
//	// more accurate, but not SIMD -- other people had far more fun coming up with solutions to this than me
//	for (int i = 0; i < 8; i++)
//	{
//		x.m256_f32[i] = powf(x.m256_f32[i], y.m256_f32[i]);
//	}
//	return x;
//
...
__forceinline Colour8 operator * (const __m256 f, const Colour8& c) { return { f * c.reds, f * c.greens, f * c.blues }; }
__forceinline Colour8 operator * (const Colour8& c, const __m256 f) { return { f * c.reds, f * c.greens, f * c.blues }; }
__forceinline Colour8 operator + (const Colour8& c1, const Colour8& c2) { return { c1.reds + c2.reds, c1.greens + c2.greens, c1.blues + c2.blues }; }
__forceinline Colour8 operator * (const Colour8& c1, const Colour8& c2) { return { c1.reds * c2.reds, c1.greens * c2.greens, c1.blues * c2.blues }; }

__forceinline Colour8 select(__m256 cond, const Colour8& c1, const Colour8& c2)
{
	return { select(cond, c1.reds, c2.reds), select(cond, c1.greens, c2.greens), select(cond, c1.blues, c2.blues) };
}

__forceinline Colour8 select(__m256i cond, const Colour8& c1, const Colour8& c2)
{
	return { select(_mm256_castsi256_ps(cond), c1.reds, c2.reds), select(_mm256_castsi256_ps(cond), c1.greens, c2.greens), select(_mm256_castsi256_ps(cond), c1.blues, c2.blues) };
}
...
__forceinline __m256 invsqrtf(const __m256& x)
{
	// for perfect imagemagick diffs, need to match accuracy of original code
	const __m256 ones = _mm256_set1_ps(1.0f);
	return ones / _mm256_sqrt_ps(x);
...
__forceinline __m256 clamp01(__m256 x)
{
	const __m256 zeros = _mm256_set1_ps(0.0f);
	const __m256 ones = _mm256_set1_ps(1.0f);
...
__forceinline __m256 lerp(__m256 x, __m256 y, __m256 t)
{
	const __m256 ones = _mm256_set1_ps(1.0f);
	return x * (ones - t) + y * t;
}

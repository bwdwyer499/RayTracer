...
void simdifySceneContainer(Scene& scene)
{
	...
	// mathemagical way of calculating ceilf(scene.numObjects / 8.0f)
	scene.numObjectsSIMD = (((int)scene.numObjects) - 1) / valuesPerVector + 1;

	scene.numObjectsSIMD = (scene.numObjects) / valuesPerVector;					// WRONG for all values _except_ for those equally divisible by 8
	scene.numObjectsSIMD = (scene.numObjects) / valuesPerVector + 1;				// WRONG for values that are equally divisible by 8
	scene.numObjectsSIMD = (int) ceilf(scene.numObjects / (float) valuesPerVector);	// this works only for smaller ranges than integer calculation (23 bits of precision)

	scene.numObjectsSIMD = (scene.numObjects) / valuesPerVector;
	if (scene.numObjects % 8 != 0) scene.numObjectsSIMD++;
